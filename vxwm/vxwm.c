...
applyrules(Client *c)
{
	...
	/* system tray must stay on the screen viewport (not move with infinite canvas) */
...
buttonpress(XEvent *e)
{
  ...
  unsigned int i, x, click, occ;
	...
	/* default to "no bind matches" — only the real root window gets ClkRootWin.
	   Unmanaged external windows (polybar, rofi, calendar-popup, etc.) are neither
	   selmon->barwin nor a Client below, and must NOT fall back to ClkRootWin or
	   their own clicks get swallowed by root binds (e.g. plain-Button1 canvas drag). */
	click = ClkLast;
...
#if INFINITE_TAGS
	/* touchpad: this ALPS PS/2 pad only reports 2-finger scroll, no real
	   swipe gesture — approximate "swipe up" as a fast burst of consecutive
	   scroll-up notches on the bare desktop. A slower/deliberate scroll still
	   just pans, via the plain SCROLL_UP->movecanvas bind in buttons[]. */
	if (click == ClkRootWin && ev->button == SCROLL_UP && CLEANMASK(ev->state) == 0) {
		...
		lastup = ev->time;
		if (burst >= SWIPE_BURST_COUNT) {
			burst = 0;
			spawn(&(Arg){.v = swipecmd});
		}
	}
#endif
...
manage(Window w, XWindowAttributes *wa)
{
  ...
  /* keep pinned clients (e.g. stalonetray) at their requested screen geometry */
  if (!c->is_pinned) {
